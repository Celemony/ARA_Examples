...
void testAudioFileChunkSaving (const PlugInEntry* plugInEntry)
{
    ...
    // create basic ARA model graph
    std::unique_ptr<TestHost> testHost;
    auto dummyAudioFiles { createDummyAudioFiles (1) };
    auto araDocumentController { createHostAndBasicDocument (plugInEntry, testHost, "testAudioFileChunkSaving", true, dummyAudioFiles) };
    auto araFactory { plugInEntry->getARAFactory () };
    const auto document { araDocumentController->getDocument () };
    ...
    for (const auto& audioSource : document->getAudioSources ())
    {
        ...
        const auto channelCount { static_cast<unsigned int> (audioSource->getChannelCount ()) };
        const auto sampleRate { static_cast<unsigned int> (audioSource->getSampleRate () + 0.5) };
        audioFile.Create (sampleCount, channelCount, sampleRate);

        // copy the audio samples from the original the audio file
...
//      ARA_INTERNAL_ASSERT (err == 0);
    ...
    testAudioFileChunkLoading (plugInEntry, audioFilesWithXMLChunk);
}
