...
// include ARADebug.h to hook into the API defined assertion function
...
std::streamsize ARAArchive::getArchiveSize () noexcept
{
    try
    {
        if (!_dataStream.good ())
...
bool ARAArchive::readBytes (std::streamoff position, std::streamsize length, char buffer[]) noexcept
{
    try
    {
        if (!_dataStream.good ())
...
bool ARAArchive::writeBytes (std::streamoff position, std::streamsize length, const char buffer[]) noexcept
{
    try
    {
        if (!_dataStream.good ())
...
ARAFileArchive::ARAFileArchive (std::string path, const std::string& documentArchiveID)
  : ARAArchive { _stream, documentArchiveID }
{
    try
    {
